        ...
        if (scrape_code.code == "success") {
            $.getJSON("/Headlines", function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    var saveHeadline = $("<a>");
                    saveHeadline.addClass("waves-effect waves-light btn save-button");
                    saveHeadline.attr("id", data[i]._id);
                    saveHeadline.text("Save Headline");
                    var byline = $("<p>");
                    byline.text(data[i].byline);
                    byline.attr("id", "byline-" + data[i]._id);
                    cardActionDiv.append(byline);
                    cardActionDiv.append(saveHeadline);
